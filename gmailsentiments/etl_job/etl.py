...
while True:
    ...
    for doc in docs:   
        ...
        try:
            df.to_sql('temp',con = pg, index=False, if_exists='append')
            #query = "INSERT INTO messages VALUES(SELECT DISTINCT * FROM temp) ON CONFLICT (id,date) DO NOTHING;"
            #pg.execute(query)
        except Exception as e:
            print(str(e))


        query = "INSERT INTO messages (SELECT DISTINCT * FROM temp) ON CONFLICT DO NOTHING;"
        pg.execute(query)
        time.sleep(3)
